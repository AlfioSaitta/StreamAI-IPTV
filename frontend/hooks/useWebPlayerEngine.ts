...
export const useWebPlayerEngine = (opts: WebPlayerEngineOptions): void => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      const [vjsModule, hlsModule, tsModule] = await Promise.all([
        import('video.js'),
        import('hls.js'),
        import('mpegts.js'),
      ]);
        ...
        probeVodSource(source, { prefetchTail: true })
          .then(result => {
            ...
            if (result.rangeSupport === 'no') {
              showOsd(createElement(AlertTriangle, { className: 'w-12 h-12 text-white' }), 'Server senza Range: seek non disponibile');
              return;
            }
            const ct = (result.contentType ?? '').toLowerCase();
            if (ct.includes('mp2t') && !detectedSource.isLive) {
              showOsd(createElement(AlertTriangle, { className: 'w-12 h-12 text-white' }), 'MPEG-TS senza indici: il seek può essere lento');
            }
          })
          .catch(() => undefined);
      }

      const bypassVideojsSource = detectedSource.engine === 'videojs' && detectedSource.protocol === 'mkv';
      const shouldDeclareType = detectedSource.engine === 'videojs'
      ...
      const updateAudioTracksList = () => {
        ...
        for (let i = 0; i < tracks.length; i++) {
          tracksList.push({ id: tracks[i].id, label: tracks[i].label, language: tracks[i].language, enabled: tracks[i].enabled });
        }
      ...
      player.on('waiting', () => { if (!player.isDisposed()) setIsBuffering(true); });
      player.on('playing', () => {
        if (!player.isDisposed()) {
          if (loadTimeoutRef.current) { window.clearTimeout(loadTimeoutRef.current); loadTimeoutRef.current = null; }
          setIsBuffering(false); broadcastStatus();
        }
      });
      player.on('seeked', () => { if (!player.isDisposed()) broadcastStatus(); });
      player.on('volumechange', () => {
        if (!player.isDisposed()) {
          setVolume(player.volume() || 0); setIsMuted(player.muted() || false); broadcastStatus();
        }
      });
      ...
      player.on('durationchange', () => {
        if (!player.isDisposed()) { setDuration(player.duration() || 0); broadcastStatus(); }
      });
      ...
      player.on('loadedmetadata', () => {
        ...
        setDuration(dur);
        if (initialProgress && initialProgress > 0.05 && initialProgress < 0.95) player.currentTime(dur * initialProgress);
        streamInfoService.collectInfoAsync(videoEl, hlsRef.current, mpegtsRef.current, source).then(info => {
          if (info.isHEVC && !info.isSupported) showOsd(createElement(AlertTriangle, { className: 'w-12 h-12 text-white' }), 'HEVC/H.265: verifica codec');
        }).catch(() => undefined);
        player.play()?.catch(e => console.warn('Autoplay bloccato:', e));
        broadcastStatus();
      });
      player.on('ended', () => { if (onNext) onNext(); broadcastStatus(); });
      player.on('error', () => {
        ...
        setPlaybackError(classifiedError); setError(classifiedError.message);
        showOsd(createElement(AlertTriangle, { className: 'w-12 h-12 text-white' }), classifiedError.title);
        scheduleRetry(classifiedError); broadcastStatus();
      });
      ...
      if (platformService.isDesktop && host) {
        networkStatusIntervalRef.current = window.setInterval(() => {
          if (player && !player.isDisposed()) {
            ...
            if (stats && stats.bandwidth) setNetworkSpeed(stats.bandwidth / 1024 / 1024);
            broadcastStatus();
          }
        }, 5000);
      }
      ...
      cleanupFn = () => {
        if (networkStatusIntervalRef.current) { window.clearInterval(networkStatusIntervalRef.current); networkStatusIntervalRef.current = null; }
