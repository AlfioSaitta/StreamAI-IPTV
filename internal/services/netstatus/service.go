...
type Service struct {
	...
	lastSent    []byte
	stopped     bool
	// multicastWarned evita di ripetere a ogni tick del rejoinLoop il warning
	// sull'impossibilità di fare il join multicast.
	multicastWarned bool
}

// logMulticastUnavailableOnce logga una sola volta che il join multicast non è
// disponibile su questa piattaforma: la ricezione degli status dei peer resta
// garantita dal fallback broadcast, ma l'utente deve poterlo vedere nei log
// invece di scoprire in silenzio che il multicast non è attivo.
func (s *Service) logMulticastUnavailableOnce(cause error) {
	s.mu.Lock()
	alreadyLogged := s.multicastWarned
	s.multicastWarned = true
	s.mu.Unlock()
	if alreadyLogged {
		return
	}
	log.Warn().Err(cause).Msg("netstatus: multicast join unavailable; peer status relies on the broadcast fallback")
}
...
// joinAllInterfaces fa join al gruppo multicast su tutte le NIC IPv4 up &
// non-loopback. È idempotente (il kernel ignora doppi join).
//
// Il socket del listener è un normale UDP wildcard: senza `IP_ADD_MEMBERSHIP` il
// kernel non gli consegna i datagrammi multicast, quindi questa funzione è
// indispensabile per la ricezione su 239.255.255.251 e non un'ottimizzazione.
// Viene richiamata all'avvio e periodicamente, per coprire i cambi di NIC.
func (s *Service) joinAllInterfaces(pc *net.UDPConn) {
	...
	if err != nil {
		log.Warn().Err(err).Msg("netstatus: cannot enumerate interfaces for multicast join")
		return
	}
	joined := 0
	var lastErr error
	for i := range ifaces {
		...
		if err := joinMulticastGroup(pc, mAddr, &iface); err != nil {
			lastErr = err
			continue
		}
		joined++
	}
	// Log una volta per esito negativo (tipicamente piattaforme senza
	// implementazione): la ricezione resta garantita dal fallback broadcast, ma
	// l'utente deve poterlo vedere nei log invece di scoprire in silenzio che il
	// multicast non è attivo.
	if joined == 0 {
		s.logMulticastUnavailableOnce(lastErr)
	}
}
