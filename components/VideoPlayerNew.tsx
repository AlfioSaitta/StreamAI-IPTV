...
const VideoPlayerNew: React.FC<VideoPlayerProps> = ({
  ...
  useEffect(() => {
    ...
    if (useVideoJS) {
      ...
      const options: any = {
        ...
        liveui: isLive,
        liveTracker: isLive ? { trackingThreshold: 0, liveTolerance: 15 } : undefined,
      };
  ...
  const setupPlayerEvents = (player: Player) => {
    ...
    player.on('error', () => {
      ...
      if (err) {
        switch (err.code) {
          case 1: errorMsg = 'Riproduzione interrotta'; break;
          case 2: errorMsg = 'Errore di rete'; break;
          case 3: errorMsg = 'Codec non supportato'; break;
  ...
  const setupNativeVideoEvents = (videoEl: HTMLVideoElement, source: string, isLive: boolean) => {
    videoEl.onplay = () => { setIsPlaying(true); setIsBuffering(false); };
    videoEl.onpause = () => setIsPlaying(false);
    videoEl.onwaiting = () => setIsBuffering(true);
    videoEl.oncanplay = () => setIsBuffering(false);
    videoEl.onplaying = () => {
      setIsBuffering(false);
      setIsPlaying(true);
      setTimeout(() => detectNativeCodecInfo(videoEl, source), 500);
    };
    ...
    videoEl.onloadedmetadata = () => {
      if (isFinite(videoEl.duration)) {
        ...
        videoEl.play().catch((err) => {
          ...
          if (err.name !== 'NotAllowedError') handlePlaybackError(err, source);
        });
      }
    };
    videoEl.onprogress = () => {
    ...
    videoEl.onerror = () => handlePlaybackError(videoEl.error, source);
  };
  ...
  const handlePlaybackError = (err: any, source: string) => {
    ...
    if (err instanceof MediaError || err?.code) {
      ...
      switch (code) {
        ...
        case MediaError.MEDIA_ERR_DECODE: errorMsg = 'Codec non supportato'; break;
        case MediaError.MEDIA_ERR_SRC_NOT_SUPPORTED: errorMsg = 'Formato non supportato'; break;
      }
    }
    setError(errorMsg);
    setIsBuffering(false);
      ...
      switch (e.key) {
        ...
        case 'ArrowUp':
          e.preventDefault();
          {
            ...
            else if (playerRef.current) playerRef.current.volume(Math.min(1, (volume || 0) + 0.1));
          }
          break;
        case 'ArrowDown':
          e.preventDefault();
          {
            ...
            else if (playerRef.current) playerRef.current.volume(Math.max(0, (volume || 0) - 0.1));
          }
          break;
    ...
    >
      ...
      {castSession.isConnected && (
        <div className="absolute inset-0 z-[100] flex flex-col items-center justify-center bg-gradient-to-b from-black/90 via-black/70 to-black/90">
          ...
          <h2 className="text-2xl font-semibold text-white mb-2">Trasmissione su {castSession.device?.name || 'dispositivo'}</h2>
          ...
          <div className="flex items-center gap-6 mb-8">
            <button onClick={() => castSession.seek(Math.max(0, castSession.status.currentTime - 10))} className="p-3 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors" title="Indietro 10s">
              <Rewind className="w-6 h-6" />
            </button>
            <button onClick={async () => { if (castSession.status.playerState === 'PLAYING') await castSession.pause(); else await castSession.play(); }} className="p-5 rounded-full bg-white text-black hover:bg-gray-200 transition-colors shadow-lg">
              {castSession.status.playerState === 'PLAYING' ? <Pause className="w-8 h-8" /> : <Play className="w-8 h-8 ml-1" />}
            </button>
            <button onClick={() => castSession.seek(castSession.status.currentTime + 10)} className="p-3 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors" title="Avanti 10s">
              <FastForward className="w-6 h-6" />
            </button>
            <button onClick={() => castSession.stop()} className="p-3 rounded-full bg-white/10 hover:bg-white/20 text-white transition-colors" title="Stop">
              <StopCircle className="w-6 h-6" />
            </button>
          </div>
          <div className="flex items-center gap-4 w-full max-w-xs px-8 mb-8">
            <button onClick={() => castSession.setMuted(!castSession.status.muted)} className="p-2 rounded-full hover:bg-white/10 text-white transition-colors">
              {castSession.status.muted || castSession.status.volume === 0 ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}
            </button>
            <input
              type="range"
              min="0"
              ...
              onChange={(e) => {
                ...
                if (val > 0 && castSession.status.muted) castSession.setMuted(false);
              }}
              className="flex-1 h-2 bg-white/20 rounded-full appearance-none cursor-pointer"
            />
            <span className="text-sm text-gray-400 w-10 text-right">{Math.round((castSession.status.muted ? 0 : castSession.status.volume) * 100)}%</span>
          </div>
          <button onClick={() => castSession.disconnect()} className="px-6 py-2 bg-red-600 hover:bg-red-500 text-white rounded-full font-medium transition-colors flex items-center gap-2">
            <X className="w-4 h-4" />
            Disconnetti
          </button>
        </div>
      ...
      <div className={`absolute inset-0 z-30 transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
        <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/80 to-transparent">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setShowStreamInfo(!showStreamInfo)} className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
                <Info className="w-5 h-5 text-white" />
              </button>
              {playlist.length > 1 && (
                <button onClick={() => setShowPlaylist(!showPlaylist)} className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          {!isPlaying && !isBuffering && (
            <button onClick={togglePlay} className="p-6 rounded-full bg-white/20 hover:bg-white/30 transition-colors pointer-events-auto">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 to-transparent">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {onPrev && <button onClick={onPrev} className="p-2 rounded-full hover:bg-white/10 transition-colors"><SkipBack className="w-6 h-6 text-white" /></button>}
              <button onClick={() => skip(-10)} className="p-2 rounded-full hover:bg-white/10 transition-colors"><Rewind className="w-6 h-6 text-white" /></button>
              <button onClick={togglePlay} className="p-3 rounded-full bg-white/10 hover:bg-white/20 transition-colors">{isPlaying ? <Pause className="w-8 h-8 text-white" /> : <Play className="w-8 h-8 text-white" fill="white" />}</button>
              <button onClick={() => skip(10)} className="p-2 rounded-full hover:bg-white/10 transition-colors"><FastForward className="w-6 h-6 text-white" /></button>
              {onNext && <button onClick={onNext} className="p-2 rounded-full hover:bg-white/10 transition-colors"><SkipForward className="w-6 h-6 text-white" /></button>}
              <div className="flex items-center gap-2 ml-4">
                <button onClick={toggleMute} className="p-2 rounded-full hover:bg-white/10 transition-colors">{isMuted || volume === 0 ? <VolumeX className="w-6 h-6 text-white" /> : <Volume2 className="w-6 h-6 text-white" />}</button>
                <input type="range" min={0} max={1} step={0.05} value={isMuted ? 0 : volume} onChange={handleVolumeChange} className="w-20 h-1 bg-white/20 rounded-full appearance-none cursor-pointer" />
              </div>
            </div>
            <div className="flex items-center gap-2">
              {availableQualities.length > 1 && (
                <div className="relative">
                  <button onClick={() => setShowQualityMenu(!showQualityMenu)} className="px-2 py-1 rounded hover:bg-white/10 transition-colors text-white text-sm font-medium">{currentQuality}</button>
                  {showQualityMenu && (
                    <div className="absolute bottom-full right-0 mb-2 bg-black/95 rounded-lg border border-white/10 overflow-hidden min-w-[120px]">
                      {availableQualities.map((q) => (
                        <button key={q.label} onClick={() => changeQuality(q.height)} className={`w-full px-4 py-2 text-left text-sm hover:bg-white/10 transition-colors ${currentQuality === q.label ? 'text-green-400' : 'text-white'}`}>
                          {q.label}
                          {q.bitrate > 0 && <span className="text-white/50 ml-2 text-xs">{(q.bitrate / 1000000).toFixed(1)}M</span>}
                        </button>
                      ))}
                    </div>
                  )}
              ...
              {channel.type !== 'live' && (
                <div className="relative">
                  <button onClick={() => setShowSpeedMenu(!showSpeedMenu)} className="px-2 py-1 rounded hover:bg-white/10 transition-colors text-white text-sm font-medium" title="Velocità di riproduzione">{playbackRate === 1 ? '1x' : `${playbackRate}x`}</button>
                  {showSpeedMenu && (
                    <div className="absolute bottom-full right-0 mb-2 bg-black/95 rounded-lg border border-white/10 overflow-hidden min-w-[80px]">
                      {playbackRates.map((rate) => (
                        <button key={rate} onClick={() => changePlaybackRate(rate)} className={`w-full px-4 py-2 text-left text-sm hover:bg-white/10 transition-colors ${playbackRate === rate ? 'text-green-400' : 'text-white'}`}>{rate === 1 ? 'Normale' : `${rate}x`}</button>
                      ))}
                    </div>
                  )}
                </div>
              )}
              {onResetProgress && channel.type !== 'live' && <button onClick={onResetProgress} className="p-2 rounded-full hover:bg-white/10 transition-colors" title="Ricomincia dall'inizio"><RotateCcw className="w-5 h-5 text-white" /></button>}
              {!platformService.isNative && <button onClick={openExternalPlayer} className="p-2 rounded-full hover:bg-white/10 transition-colors" title="Apri con player esterno (VLC)"><ExternalLink className="w-5 h-5 text-white" /></button>}
              <button onClick={() => castSession.isConnected ? null : setShowDevicePicker(true)} disabled={isCastLoading || castSession.isConnecting} className={`p-2 rounded-full hover:bg-white/10 transition-colors ${castSession.isConnected ? 'text-blue-400' : 'text-white'} ${isCastLoading || castSession.isConnecting ? 'opacity-50' : ''}`} title={castSession.isConnected ? `Casting su ${castSession.device?.name}` : 'Trasmetti'}>{isCastLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Cast className="w-5 h-5" />}</button>
              {'pictureInPictureEnabled' in document && <button onClick={togglePiP} className="p-2 rounded-full hover:bg-white/10 transition-colors"><PictureInPicture2 className="w-5 h-5 text-white" /></button>}
              <button onClick={toggleFullscreen} className="p-2 rounded-full hover:bg-white/10 transition-colors">{isFullscreen ? <Minimize className="w-6 h-6 text-white" /> : <Maximize className="w-6 h-6 text-white" />}</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {channel && (
        <CastDevicePicker
          ...
          onCastStart={() => setIsCastLoading(true)}
          onCastSuccess={() => setIsCastLoading(false)}
          onCastError={() => setIsCastLoading(false)}
        />
      )}
    </div>
  );
