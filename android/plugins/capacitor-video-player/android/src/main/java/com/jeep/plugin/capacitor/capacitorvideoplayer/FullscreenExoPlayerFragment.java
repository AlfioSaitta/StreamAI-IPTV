...
// MED-1: MediaSessionCompat sostituito da androidx.media3.session.MediaSession
...
// MED-1: androidx.media3.session.MediaSession sostituisce MediaSessionConnector di ExoPlayer 2
...
// MED-1: Media3 espone Util in androidx.media3.common.util (marker @UnstableApi)
...
public class FullscreenExoPlayerFragment extends Fragment {
  ...
  private final List<String> supportedFormat = Arrays.asList(
    // MED-1 (Step 3-ter): MKV/Matroska riconosciuto da Media3 via MatroskaExtractor
    // di default; MOV/M4V via Mp4Extractor; MP3/AAC/FLAC/WAV via extractor dedicati.
    new String[] {
      "mp4", "m4v", "mov",
      "webm", "mkv", "matroska",
      "ogv", "ogg",
  ...
  // MED-1: MediaSession Media3 auto-bound al Player (sostituisce MediaSessionCompat + MediaSessionConnector)
...
*/
  ...
  public void releasePlayer() {
    if (player != null) {
      ...
      // MED-1: rilascio MediaSession Media3
  ...
  private void initializePlayer() {
    if (player == null) {
      ...
      // MED-1 (Step 3-bis): preferenze lingua + tunneling HW per HDR/4K
      ...
      // MED-1 (Step 3-bis): buffer tuning IPTV-friendly (min 15s / max 50s / playback 1.5s / rebuffer 5s)
      LoadControl loadControl = new DefaultLoadControl.Builder()
        .setBufferDurationsMs(15_000, 50_000, 1_500, 5_000)
        .setPrioritizeTimeOverSizeThresholds(true)
        .build();
      // MED-1 (Step 3-bis): renderers factory con decoder fallback (HEVC/AV1 graceful) e extension renderer prefer
    ...
    if (mediaSource != null) {
      ...
      // MED-1: prepare(MediaSource, ...) rimosso in Media3, usare setMediaSource + prepare
    ...
    // MED-1 (Step 3): MediaSession Media3 — auto-bound al Player, gestisce lock-screen / BT / cuffie.
    // Sostituisce MediaSessionCompat + MediaSessionConnector di ExoPlayer 2 (deprecati).
    if (mediaSession != null) {
      mediaSession.release();
    }
    mediaSession = new MediaSession.Builder(context, player)
      .setId("streamai-fullscreen-player-" + (playerId != null ? playerId : "default"))
      .build();
  ...
  private MediaSource buildAssetMediaSource(Uri uri) {
    ...
    DataSource.Factory dataSourceFactory = new DefaultDataSource.Factory(context, new DefaultHttpDataSource.Factory().setUserAgent("jeep-exoplayer-plugin"));
  ...
  private MediaSource buildHttpMediaSource() {
    ...
    if (
      ...
      // MED-1 (Step 3-ter): ProgressiveMediaSource sceglie l'extractor giusto
      // (Mp4/Matroska/Ogg/Flv/Ts/Mp3/Flac/Wav/AdtsAac) via DefaultExtractorsFactory.
      mediaSource = new ProgressiveMediaSource.Factory(dataSourceFactory).createMediaSource(MediaItem.fromUri(uri));
      ...
      mediaSource = new HlsMediaSource.Factory(dataSourceFactory).setAllowChunklessPreparation(true).createMediaSource(MediaItem.fromUri(uri));
